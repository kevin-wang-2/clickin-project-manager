...
| 重构 | `refactor/` | `refactor/internal-user-id` |
...
1. **开 Issue** → 在 GitHub Issues 描述问题或需求，打 label，关联 milestone
2. **创建分支** → 从 `main` 拉取，按命名规范命名
...
4. **CI 自动触发**：
   ...
   - `unit-test`：Vitest 单元测试（含 migration 测试）
5. **自动请求 Review**：CODEOWNERS（`.github/CODEOWNERS`）配置 `* @kevin-wang-2 @Click-In-Studio/reviewer`，PR 创建后自动 request review
...
合并到 `main` 后 CI/CD 自动完成所有操作：构建、上传、DB migration、切换 release、pm2 热重启。

**无需任何手动操作**，详见 [部署流程](./DEPLOY.md)。
...
优先级对应 §11.1 的两级原则：**Top** 保数据完整性，**P1** 保安全性。
